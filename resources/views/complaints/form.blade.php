    ...
    <div class="space-y-6" x-data="{
       ...
       <x-slot name="header">
        <h1 class="text-xl font-semibold text-gray-800 dark:text-gray-200 flex items-center gap-2">
            {{-- Ikon berubah sesuai kategori --}}
            @php
                $icons = [
                    'rutilahu' => 'fa-house-damage',
                    'infrastruktur' => 'fa-road',
                    'tata_kota' => 'fa-city',
                    'air_bersih_sanitasi' => 'fa-tint',
                ];
                $icon = $icons[$category] ?? 'fa-file-alt';
            @endphp
            <i class="fas {{ $icon }} text-indigo-500"></i>
            <span>Formulir Pengaduan: {{ ucwords(str_replace('_', ' ', $category)) }}</span>
        </h1>
    </x-slot>
